  ...
  test "<%= action_first_word %>/1 with valid data creates a <%= contract.schema.singular %>" do
    ...
    assert {:ok, %<%= inspect contract.schema.alias %>{} = <%= contract.schema.singular %>} = <%= action_capitalized %><%= inspect(contract.schema.alias) %>.<%= action_first_word %>(valid_attrs)
  ...
  test "<%= action_first_word %>/1 with invalid data returns error changeset" do
    assert {:error, %Ecto.Changeset{}} = <%= action_capitalized %><%= inspect(contract.schema.alias) %>.<%= action_first_word %>(@invalid_attrs)
  end
